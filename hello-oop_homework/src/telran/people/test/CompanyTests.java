...
class CompanyTests {
	...
	void testFindSalesPersons() {
		...
		assertArrayEquals(expected, company.findEmployees((Employee t)-> t instanceof SalesPerson));
	}
	@Test
	void testFindEmployeesSalaryRange() {
		...
		assertArrayEquals(expectedGT10000, 
				company.findEmployees((Employee t)->{
					int salary = t.computePay();
					return salary >= 10000 && salary <= Integer.MAX_VALUE;
				}));
		...
		assertArrayEquals(expected1000_1500,
				company.findEmployees((Employee t)->{
					int salary = t.computePay();
					return salary >= 1000 && salary <= 1500;
				}));
	}
	@Test
